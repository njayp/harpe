...
type FontChunk = { title: string; pageStart: number; lines: Line[] };
...
export function detectHeadings(
  ...
  let current: FontChunk | null = null;
  let prevHeading: Line | null = null;
...
function isContinuationLine(
  line: Line,
  prev: Line,
  callerRule?: (text: string) => boolean,
): boolean {
  if (callerRule) return callerRule(line.text);
  if (line.page !== prev.page) return false;
  const dy = prev.y - line.y;
  if (dy <= 0) return false;
  return dy < line.fontSize * 2.0;
}

function chunkLengths(chunks: FontChunk[]): number[] {
  return chunks.map((c) => {
    let len = 0;
    for (const l of c.lines) len += l.text.length + 1;
    return Math.max(0, len - 1);
  });
}

function chunksToSections(chunks: FontChunk[], pageCount: number): Section[] {
...
export function byFont(lines: Line[], pageCount: number, opts: ExtractOptions = {}): Section[] {
  ...
  if (typeof opts.ratio === 'number') {
    return chunksToSections(
      detectHeadings(lines, opts.ratio, body, opts.isContinuation),
      pageCount,
    );
  }

  let best: { score: number; chunks: FontChunk[] } = { score: -Infinity, chunks: [] };
  for (const ratio of DEFAULT_RATIOS) {
    ...
    if (score > best.score) best = { score, chunks };
  }
  return chunksToSections(best.chunks, pageCount);
}

export function byOutline(
  lines: Line[],
...
function dedupeOutline(outline: OutlineEntry[]): OutlineEntry[] {
...
function groupLinesByPage(lines: Line[]): Map<number, Line[]> {
...
function collectText(linesByPage: Map<number, Line[]>, start: number, endExclusive: number): string {
  const parts: string[] = [];
  for (let p = start; p < endExclusive; p++) {
    const pageLines = linesByPage.get(p);
    if (!pageLines) continue;
    for (const line of pageLines) parts.push(line.text);
  }
  return parts.join('\n').trim();
}

export function sanityCheck(sections: Section[], minSectionChars: number): void {
...
export function shouldUseOutline(outline: OutlineEntry[]): boolean {
